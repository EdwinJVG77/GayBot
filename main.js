...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "GayJadiBot "], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
console.log(chalk.bold.redBright(lenguajeGB['smsConexionOFF'](reason, connection)))
await global.reloadHandler(true).catch(console.error);
}}}
...
let prekey = []
let directorio = readdirSync("./GayBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') || file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./GayBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./GayJadiBot /');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./GayJadiBot /${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./GayJadiBot /${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-') || fileInDir.startsWith('app-') || fileInDir.startsWith('session-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./GayJadiBot /${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./GayBotSession/', './GayJadiBot /']
